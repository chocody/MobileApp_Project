...
class EventService {
  ...
  //add new event
  Future<void> addEvent(gid,eventName, description,date,time, Uint8List file) async{
    ...
    for (Map<String, dynamic> user in userList){
      ...
      await _firestore.collection("user_event").doc(addedEvent.id + user["uid"]).set(relation);
    }
  ...
  Stream<List<Map<String,dynamic>>> getEventsStream(){
    return _firestore.collection("user_event").where("uid", isEqualTo: _auth.currentUser!.uid).snapshots().map((snapshot){
